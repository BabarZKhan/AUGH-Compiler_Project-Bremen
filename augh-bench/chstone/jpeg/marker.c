...
/*
 *  Index to read buffer JpegFileBuf
 */
static unsigned ReadBuf_idx;
 ...
 */
int read_byte (void) {
	return JpegFileBuf[ReadBuf_idx++];
}
...
short read_word (void) {
	...
	c = JpegFileBuf[ReadBuf_idx++];
	c = (c << 8) | JpegFileBuf[ReadBuf_idx++];
...
void get_sof () {
	int ci, c;
	int length;
	unsigned i_comp_info_index;          // Index in array p_jinfo_comps_info_index declared in decode.h
	unsigned i_comp_info_id;             // Index in array p_jinfo_comps_info_id
	unsigned i_comp_info_h_samp_factor;  // Index in array p_jinfo_comps_info_h_samp_factor
	unsigned i_comp_info_v_samp_factor;  // Index in array p_jinfo_comps_info_v_samp_factor
	unsigned i_comp_info_quant_tbl_no;   // Index in array p_jinfo_comps_info_quant_tbl_no
	...
	for (ci = 0; ci < p_jinfo_num_components; ci++) {
		i_comp_info_index         = ci;
		i_comp_info_id            = ci;
		i_comp_info_h_samp_factor = ci;
		i_comp_info_v_samp_factor = ci;
		i_comp_info_quant_tbl_no  = ci;

		p_jinfo_comps_info_index[i_comp_info_index] = ci;
		p_jinfo_comps_info_id[i_comp_info_id] = read_byte ();
		c = read_byte ();
		p_jinfo_comps_info_h_samp_factor[i_comp_info_h_samp_factor] = (c >> 4) & 15;
		p_jinfo_comps_info_v_samp_factor[i_comp_info_v_samp_factor] = (c) & 15;
		p_jinfo_comps_info_quant_tbl_no[i_comp_info_quant_tbl_no] = read_byte ();

		printf (" index         = %d\n", p_jinfo_comps_info_index[i_comp_info_index]);
		printf (" id            = %d\n", p_jinfo_comps_info_id[i_comp_info_id]);
		printf (" h_samp_factor = %d\n", p_jinfo_comps_info_h_samp_factor[i_comp_info_h_samp_factor]);
		printf (" v_samp_factor = %d\n", p_jinfo_comps_info_v_samp_factor[i_comp_info_v_samp_factor]);
		printf (" quant_tbl_no  = %d\n\n", p_jinfo_comps_info_quant_tbl_no[i_comp_info_quant_tbl_no]);

		if (p_jinfo_comps_info_index[i_comp_info_index] != out_index_get_sof[ci]) {
			main_result++;
		}
		if (p_jinfo_comps_info_id[i_comp_info_id] != out_id_get_sof[ci]) {
			main_result++;
		}
		if (p_jinfo_comps_info_h_samp_factor[i_comp_info_h_samp_factor] != out_h_samp_factor_get_sof[ci]) {
			main_result++;
		}
		if (p_jinfo_comps_info_v_samp_factor[i_comp_info_v_samp_factor] != out_v_samp_factor_get_sof[ci]) {
			main_result++;
		}
		if (p_jinfo_comps_info_quant_tbl_no[i_comp_info_quant_tbl_no] != out_quant_tbl_no_get_sof[ci]) {
			main_result++;
		}
...
void get_sos () {
  ...
  int i, c, cc, ci, j;
  unsigned i_comp_info_id;         // Index in array p_jinfo_comps_info_id
  unsigned i_comp_info_dc_tbl_no;  // Index in array p_jinfo_comps_info_dc_tbl_no
  unsigned i_comp_info_ac_tbl_no;  // Index in array p_jinfo_comps_info_ac_tbl_no
  ...
  for (i = 0; i < num_comp; i++) {
		...
		for (ci = 0; ci < p_jinfo_num_components; ci++) {
			i_comp_info_id = ci;
			i_comp_info_dc_tbl_no = ci;
			i_comp_info_ac_tbl_no = ci;

			if (cc == p_jinfo_comps_info_id[i_comp_info_id])
				goto id_found;
		}
		main_result++;
    ...
    id_found:
		...
		p_jinfo_comps_info_dc_tbl_no[i_comp_info_dc_tbl_no] = (c >> 4) & 15;
		p_jinfo_comps_info_ac_tbl_no[i_comp_info_ac_tbl_no] = (c) & 15;

		printf (" comp_id       = %d\n", cc);
		printf (" dc_tbl_no     = %d\n", p_jinfo_comps_info_dc_tbl_no[i_comp_info_dc_tbl_no]);
		printf (" ac_tbl_no     = %d\n", p_jinfo_comps_info_ac_tbl_no[i_comp_info_ac_tbl_no]);

		if (cc != out_comp_id_get_sos[i_get_sos]) {
			main_result++;
		}
    if (p_jinfo_comps_info_dc_tbl_no[i_comp_info_dc_tbl_no] != out_dc_tbl_no_get_sos[i_get_sos]) {
			main_result++;
		}
    if (p_jinfo_comps_info_ac_tbl_no[i_comp_info_ac_tbl_no] != out_ac_tbl_no_get_sos[i_get_sos]) {
			main_result++;
		}
		i_get_sos++;
  ...
  /*
   * Define the Buffer at this point as the head of data, in buffer JpegFileBuf
   */
...
void get_dht () {
	...
	while (length > 16) {
		...
		for (i = 1; i <= 16; i++) {
			value = read_byte ();
			if(is_ac!=0) p_jinfo_ac_xhuff_tbl_bits[index][i] = value;
			else         p_jinfo_dc_xhuff_tbl_bits[index][i] = value;
			count += value;
		}
		...
		for (i = 0; i < count; i++) {
			value = read_byte ();
			if(is_ac!=0) p_jinfo_ac_xhuff_tbl_huffval[index][i] = value;
			else         p_jinfo_dc_xhuff_tbl_huffval[index][i] = value;
		}
...
void get_dqt () {
	...
	while (length > 0) {
		...
		for (i = 0; i < DCTSIZE2; i++) {
			if (prec) tmp = read_word ();
			else      tmp = read_byte ();
			p_jinfo_quant_tbl_quantval[num+1][izigzag_index[i]] = (unsigned short) tmp;
		}
