...
#endif
{
	...
	while (1) {
		...
		for (i = 0; i < 8; i++) dmem[i] = A[i];
		// FIXME This initialization to zero was not in CHStone original code but it should be!
		for (     ; i < 64; i++) dmem[i] = 0;
		...
		for (j = 0; j < 8; j++) main_result += (dmem[j] != outData[j]);
